export function renderFeature(gherkinDocument, config = {}) {
    ...
    let output = `// Generated by quickpickle
...
`;
    ...
    let { backgroundSteps, children } = renderChildren(feature.children, config);
...
function renderChildren(children, config, sp = '  ') {
    const output = {
        backgroundSteps: '',
        children: '',
    };
...
function renderRule(child, config, sp = '  ') {
    let { backgroundSteps, children } = renderChildren(child.rule.children, config, sp + '  ');
    return `
...
function renderScenario(child, config, sp = '  ') {
